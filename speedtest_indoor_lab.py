...
class Speedtest(object):
    ...
    def get_config(self):
        ...
        try:
            ...
            download['testlength'] = 1
            download['threadsperurl'] = 1
            download['initialtest'] = '35K'
            print download
    ...
    def download(self, callback=do_nothing):
        """Test download speed against speedtest.net, test ping from server"""
        ...
        def consumer(q, request_count):
            while len(finished) < request_count:
                ...
                #print('finishing consumer: ' + str(sum(thread.result)))
        ...
        while prod_thread.isAlive():
            ...
            #print('Producer index count: ' + str(index_count))
            index_count += 1
        index_count = 0
        while cons_thread.isAlive():
            cons_thread.join(timeout=0.1)
            #print('Consumer index count: ' + str(index_count))
            index_count += 1

        stop = timeit.default_timer()
        self.results.bytes_received = sum(finished)
        print ('Download time :' + str(stop - start))
        print ('Ex-ping time :' + str(stop-start-self.results.ping/1000))
        self.results.download = (
            (self.results.bytes_received / (stop - start)) * 8.0
        )
        #print('Received bytes: %0.2f M bytes' % ( sum(finished) / 1000.0 / 1000.0))
...
def parse_args():
    ...
    parser.add_argument('--timeout', default=5, type=PARSER_TYPE_FLOAT,
    ...
    parser.add_argument('--testtime', default = 100,type=PARSER_TYPE_INT,help='Total test download times in this test.')
    parser.add_argument('--theme',default=2,type=PARSER_TYPE_INT,help='Test themes for this test, 1: bus, 2: indoor_lab2, 3: outdoor')
    parser.add_argument('--looptime', default = 1000000, type=PARSER_TYPE_INT,
                        help='Test loop times for one time test')
...
def shell():
    ...
    while j < args.looptime:
        # do speed test.:
        speedtest.testPing()
        result_rows = str(SpeedtestResults.csv_header(delimiter=args.csv_delimiter)) + '\n'
        i = 0
        ...
        while i < test_time:
            # choose a download scheme
            # 350 * 4
            # 500 * 2
            # 750 + 350
            if i % 3 == 0:
                speedtest.config['sizes']['download'] = [350]
                speedtest.config['counts']['download'] = 4
            elif i % 3 == 1:
                speedtest.config['sizes']['download'] = [500]
                speedtest.config['counts']['download'] = 2
            elif i % 3 == 2:
                speedtest.config['sizes']['download'] = [750]
                speedtest.config['counts']['download'] = 1

            cur_timestamp = datetime.datetime.now()
            delta = cur_timestamp - prev_timestamp
            if delta.seconds < time_interval:
                time.sleep(time_interval - delta.seconds)
            prev_timestamp = datetime.datetime.now()
            printer('Testing download speed: ' + str(i), quiet,
                    end=('', '\n')[bool(debug)])
            speedtest.results.timestamp = datetime.datetime.now().isoformat()
            ...
            printer('Download: %0.2f M%s/s' %
                    ((results.download / 1000.0 / 1000.0) / args.units[1],
                     args.units[0]),
                    quiet)
            if results.download < 1:
                if retry_times < 1:
                    speedtest.config['sizes']['download'] = [1000]
                    speedtest.config['counts']['download'] = 1
                    retry_times += 1
                    continue
            retry_times = 0
            #print('Ping: ' + str(results.ping) + 'ms')
            total_received += results.bytes_received
            #print('Received bytes: %0.2f M bytes' % ( results.bytes_received / 1000.0 / 1000.0))
            i += 1
            #printer(results.csv(delimiter=args.csv_delimiter))
            result_rows = result_rows + str(results.csv(delimiter=args.csv_delimiter)) + '\n'

        # send results.
        #sendEmail(result_rows)
        print('Received bytes: %0.2f M bytes' % ( total_received / 1000.0 / 1000.0))
        sendResults(result_rows)
        j += 1
#
    # if args.upload:
...
def sendEmail(results):
    args = parse_args()
    asender = 'tionry@163.com'
    areceiver = 'tionry@163.com'
    asubject = 'Speedtest Results'
    acc = ''

    themes = ['null','bus','indoor','outdoor']
    ...
    #attachment
    att = MIMEText(results, 'base64', 'utf-8')
    att['Content-Type'] = 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'
    # 下面的filename 等号(=)后面好像不能有空格
    time_str = str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    attname = 'attachment; filename ="result_'+ str(themes[args.theme])+ ' ' + time_str + '.csv"'
    att['Content-Disposition'] = attname
    msgroot.attach(att)

    try:
        asmtp = smtplib.SMTP(asmtpserver, 25, timeout=500)
        ...
        asmtp.quit()
        printer('Finish send email.')
    except e:
        printer('send email error:' + str(e))

def sendResults(result_rows):
    args = parse_args()
    themes = ['null', 'bus', 'indoor_lab', 'outdoor']
    url = 'http://47.94.108.78/file-upload'

    result_str = result_rows
    time_str = str(datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S'))
    attname = 'result_' + str(themes[args.theme]) + ' ' + time_str + '.csv'
    file = attname
    file_type = str(themes[args.theme])
